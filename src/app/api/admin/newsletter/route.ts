...
export async function GET() {
  ...
  return NextResponse.json(await prisma.newsletter.findMany({ orderBy:{ createdAt:'desc' } }));
}
export async function POST(req: NextRequest) {
  ...
  const record = await prisma.newsletter.create({ data:{ subject, content, sentAt:new Date(), sentCount:subscribers.length } });
